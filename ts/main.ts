...
function displayMovieList(movies: Movie[]): void {
  if ($searchList) {
    // 显示搜索结果的列表
...
function createFlipCardHome(data: any): HTMLElement {
...
async function renderHome(): Promise<void> {
  ...
  try {
    const response = await fetchData<TMDBResponse>(url);
...
// 加载电影详细信息
function loadMovieDetails(): void {
  if ($searchList) {
...
function addToFavorites(details: MovieDetails): void {
  console.log('Adding movie:', details);

  const favorites: MovieDetails[] = JSON.parse(
...
// 显示电影详细信息
function displayMovieDetails(details: MovieDetails): void {
  if (!resultGrid) {
  ...
  // 确保按钮存在并添加事件监听
  const favButton = resultGrid.querySelector(
    '.favorite-btn',
